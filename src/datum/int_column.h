...
#ifndef SANDDB_INT_COLUMN_H
#define SANDDB_INT_COLUMN_H

#include "column.h"

class IntColumn :   public  ColumnBase    {
private:
    vector<std::unique_ptr<DataVector<int>>> vectors;
public:
    IntColumn(VectorType vt);

    IntColumn();

    void addVector(std::unique_ptr<DataVector<int>> vec);
