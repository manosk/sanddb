...
#ifndef SANDDB_STRING_COLUMN_H
#define SANDDB_STRING_COLUMN_H

#include "column.h"

class StringColumn :   public  ColumnBase    {
private:
    vector<std::unique_ptr<DataVector<string>>> vectors;
public:
    StringColumn(VectorType vt);

    StringColumn();

    void addVector(std::unique_ptr<DataVector<string>> vec);
